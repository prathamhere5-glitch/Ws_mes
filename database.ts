...
class PostgresDB implements DB {
    ...
    prepare(sql: string) {
        ...
        if (pgSql.toUpperCase().includes("INSERT OR IGNORE")) {
            pgSql = pgSql.replace(/INSERT OR IGNORE INTO (\w+) \((.*?)\) VALUES \((.*?)\)/i, (match, table, cols, vals) => {
                const colList = cols.split(',').map((c: string) => c.trim());
                const firstCol = colList[0];
                return `INSERT INTO ${table} (${cols}) VALUES (${vals}) ON CONFLICT (${firstCol}) DO NOTHING`;
            });
        }
        ...
        if (pgSql.toUpperCase().includes("INSERT OR REPLACE")) {
            pgSql = pgSql.replace(/INSERT OR REPLACE INTO (\w+) \((.*?)\) VALUES \((.*?)\)/i, (match, table, cols, vals) => {
                const colList = cols.split(',').map((c: string) => c.trim());
                const firstCol = colList[0];
                const updateSet = colList.slice(1).map((c: string) => `${c} = EXCLUDED.${c}`).join(', ');
