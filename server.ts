...
const translations: any = {
    en: {
        ...
        force_sub: "⚠️ *Force Subscription Required*\n\nYou must join our channels before you can use this bot.",
    ...
    hi: {
        ...
        admin: "🔑 एडमिन पैनल",
        lang_btn: "🌐 भाषा बदलें",
        force_sub: "⚠️ *सब्सक्रिप्शन अनिवार्य है*\n\nइस बॉट का उपयोग करने से पहले आपको हमारे चैनलों से जुड़ना होगा।",
    ...
    id: {
        ...
        force_sub: "⚠️ *Langganan Wajib*\n\nAnda harus bergabung dengan saluran kami sebelum dapat menggunakan bot ini.",
    ...
    zh: {
        ...
        schedule_set: "✅ 定时设置成功：*{range}* (IST)。\n\n机器人现在将自动遵循此定时。",
        schedule_instruction: "📅 *设置消息定时 (IST)*\n\n请输入 24 小时格式的时间范围（例如，`09:00-18:00`）。",
        back_btn: "🔙 返回",
...
async function checkForceSub(ctx: Context, telegramId: string): Promise<boolean> {
    ...
    for (const channel of channels) {
        try {
            const member = await ctx.telegram.getChatMember(channel.channel_id, parseInt(telegramId));
            if (['left', 'kicked', 'restricted'].includes(member.status)) {
                return false;
            }
        } catch (e: any) {
            console.error(`Error checking sub for channel ${channel.channel_id}:`, e.message);
            // If bot is not admin in the channel, we can't verify.
            // To avoid locking everyone out, we'll allow it but log the error.
            // Only return false if we are CERTAIN they are not in the channel.
            if (e.message.includes('chat not found') || e.message.includes('admin privileges')) {
                console.warn(`[ForceSub] Bot lacks access to channel ${channel.channel_id}. Error: ${e.message}`);
                // If bot is not admin, notify the owner
                if (e.message.includes('admin privileges')) {
                    try {
                        await ctx.telegram.sendMessage(OWNER_ID, `⚠️ *Bot is not admin in channel:* ${channel.channel_name || channel.channel_id}\n\nPlease promote the bot to admin to enable force subscription check.`, { parse_mode: 'Markdown' });
                    } catch (err) {}
                }
                continue; 
            }
...
bot.use(async (ctx, next) => {
    if (ctx.from) {
        ...
        // Language selection check (only if not already selecting language)
        ...
        if (telegramId !== OWNER_ID) {
            try {
                const notifyMsg = `🆕 *New User Alert*\n\n` +
                                 `👤 Name: ${firstName}\n` +
                                 `🆔 ID: \`${telegramId}\`\n` +
                                 `🔗 Username: @${username ? username.replace(/_/g, '\\_') : 'N/A'}`;
...
bot.action('admin_stats', async (ctx) => {
    try {
        ...
        lastUsers.forEach(u => {
            ...
            msg += `${status}${auth} \`${u.telegram_id}\` (@${u.username || 'N/A'})\n`;
        });
...
bot.action('admin_users', async (ctx) => {
    try {
        ...
        for (const u of users) {
            ...
            msg += `${status}${admin} \`${u.telegram_id}\` (@${u.username || 'N/A'})\n`;
...
bot.action('verify_sub', async (ctx) => {
    try {
        ...
        if (isSubbed) {
            ...
            const user = await db.prepare("SELECT * FROM users WHERE telegram_id = ?").get(telegramId) as any;
            return ctx.reply("👋 Welcome! You have successfully joined the required channels.", getMainMenu(telegramId));
        } else {
            await ctx.answerCbQuery("❌ Verification Failed! You haven't joined all channels yet.", { show_alert: true });
            const user = await db.prepare("SELECT * FROM users WHERE telegram_id = ?").get(telegramId) as any;
            const lang = user?.language || 'en';
            const t = translations[lang] || translations.en;
            
            // Send a message instead of just an alert if they keep failing
            await ctx.reply("⚠️ *Verification Failed*\n\nPlease make sure you have joined all the channels listed above and then click the verify button again.", { parse_mode: 'Markdown' });
        }
    } catch (err: any) {
...
function isWithinTimeRange(rangeStr: string, language: string = 'en'): boolean {
    ...
    try {
        ...
        // For English/Hindi, use IST (UTC+5:30)
        let offset = 0;
        if (language === 'en' || language === 'hi') {
            offset = 5.5 * 60 * 60 * 1000;
        } else {
            // Default to UTC or system time for others
            offset = 0;
        }
    ...
    app.listen(Number(PORT), "0.0.0.0", async () => {
        ...
        if (botToken && botToken !== "DUMMY_TOKEN") {
            ...
            const startBot = async (retries = 5, delay = 5000) => {
                ...
                console.error("❌ [Telegram] Could not start bot after multiple attempts due to persistent 409 Conflict.");
            };

            startBot();
        } else {
            console.warn("⚠️ [Telegram] Bot token is MISSING or DUMMY. Bot will not start. Please set TELEGRAM_BOT_TOKEN in Render environment variables.");
        }
    });
}
