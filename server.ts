...
if (!botToken) {
  ...
  bot.command("help", (ctx) => {
    ...
    const helpText = `📖 *WhatsApp Bot Manager Help*
...
*Features:*
- Link multiple WhatsApp accounts via pairing code.
- Automated P2P messaging between your accounts to warm them up.
- Automated Group messaging to keep groups active.
- Anti-ban protection with human-like typing and random delays.
- Scheduling and custom delays.

Need more help? Contact @indiawsagent`;
  ...
  bot.command("status", (ctx) => {
    ...
    const connectedCount = userAccs.map(p => state.sessions.get(p)).filter(s => s && s.status === "connected").length;
    
    const statusText = t(userId, "status_text", {
      total: state.totalMessagesSent,
      sessions: `${connectedCount}/${userAccs.length}`,
      delay: state.delay,
  ...
  bot.command("ping", (ctx) => {
    ctx.reply("🏓 *Pong!* The bot is online and responsive.", { parse_mode: "Markdown" });
  });
