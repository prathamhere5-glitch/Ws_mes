...
// Force IPv4 for all network requests to avoid ENETUNREACH (IPv6 issues) on Render
...
async function connectToWhatsApp(telegramId: string, phoneNumber: string, bot: Telegraf<Context>) {
    ...
    try {
        ...
        // We no longer restore from DB to ensure fresh logins as requested
        ...
        sock.ev.on('creds.update', async () => {
            await saveCreds();
            // Also save to DB for persistence across ephemeral filesystem wipes
            const credsPath = path.join(authPath, 'creds.json');
            if (fs.existsSync(credsPath)) {
                const credsJson = fs.readFileSync(credsPath, 'utf-8');
                await db.prepare("INSERT OR REPLACE INTO whatsapp_auth (session_id, data) VALUES (?, ?)").run(sessionId, credsJson);
            }
        });
...
bot.action('admin_restore', async (ctx) => {
    try {
        ...
        await ctx.editMessageText("🔄 *Starting Manual Restoration...*\n\nRestoring all WhatsApp sessions from database and filesystem.", { parse_mode: 'Markdown', ...keyboard });
        await reconnectAllSessions(bot);
